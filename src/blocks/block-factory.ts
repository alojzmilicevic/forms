...
import type { Block, BlockType, DividerBlock, NumberInputBlock, TextBlock, TextInputBlock } from "@/manifest.type";
...
export const renderEditorBlock = ({ block, onUpdate }: RenderBlockProps): React.ReactElement | null => {
    ...
    if (isInputBlock(type)) {
        const { placeholder, ...inputProps } = blockProps as Omit<TextInputBlock | NumberInputBlock, 'type' | 'id' | 'order'>;
        return React.createElement(TextInput, {
            onChange: (e: React.ChangeEvent<HTMLInputElement>) => onUpdate({ placeholder: e.target.value } as Partial<Block>),
...
export const renderPreviewBlock = ({ block }: RenderBlockProps) => {
    ...
    if (isInputBlock(type)) {
        const inputProps = blockProps as Omit<TextInputBlock | NumberInputBlock, 'type' | 'id' | 'order'>;
        return React.createElement(TextInput, {
            variant: "outlined",
